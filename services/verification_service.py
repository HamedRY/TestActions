...
def get_verifications(args):
    ...
    response = requests.get(
        '{}/search/verifications?{}'.format(ApiAccess._swift_api_base,
...
@_internal_cache.cached()
@_test_generator.create()
def find_verification(customer_id, verification_id):
    headers = {
        'Authorization': 'Bearer {}'.format(ApiAccess.get_swift_access_token())
    }

    response = requests.get(
        '{}/customers/{}/verifications/{}'.format(
            ApiAccess._swift_api_base, customer_id, verification_id),
        headers=headers)
    response.raise_for_status()
    return response.json()
